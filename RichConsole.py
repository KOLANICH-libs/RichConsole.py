...
class RichStr:
	...
	def getCSSStyle(self) -> str:
		...
		cssSheet = {}  # type: typing.Mapping[str, str]
...
if __name__ == "__main__":
	...
	print(groups.Underline.underline(demo("https://github.com/" + __author__ + "-libs/" + thisLibName)))
	print(
		groups.Blink.rapid(
			groups.Underline.underline("Yo dawg"),
			" ",
			groups.Fore.red(groups.Back.lightcyanEx("so we heard")),
			" ",
			groups.Fore.lightyellowEx("you", groups.Fore.lightredEx(" like "), groups.Brightness.bright("text styles")),
			" so we put styles ",
			groups.Back.green("in your styles"),
			" ",
			demo("so you can style while you styling"),
			".",
		)
	)
